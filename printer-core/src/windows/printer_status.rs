use crate::{PrinterError, PrinterStatus, Result};

use std::{ffi::OsStr, iter::once};

use windows::{
    Win32::Graphics::Printing::{
        ClosePrinter, GetPrinterW, OpenPrinterW, PRINTER_ACCESS_USE, PRINTER_DEFAULTSW,
        PRINTER_INFO_2W, PRINTER_STATUS_BUSY, PRINTER_STATUS_DOOR_OPEN, PRINTER_STATUS_ERROR,
        PRINTER_STATUS_IO_ACTIVE, PRINTER_STATUS_MANUAL_FEED, PRINTER_STATUS_OFFLINE,
        PRINTER_STATUS_OUTPUT_BIN_FULL, PRINTER_STATUS_PAPER_JAM, PRINTER_STATUS_PAPER_OUT,
        PRINTER_STATUS_PAUSED, PRINTER_STATUS_PENDING_DELETION, PRINTER_STATUS_PRINTING,
        PRINTER_STATUS_PROCESSING, PRINTER_STATUS_TONER_LOW, PRINTER_STATUS_USER_INTERVENTION,
        PRINTER_STATUS_WAITING, PRINTER_STATUS_WARMING_UP,
    },
    core::PCWSTR,
};

fn to_wide(value: &str) -> Vec<u16> {
    value.encode_utf16().chain(std::iter::once(0)).collect()
}

pub fn get_printer_status(printer: &str) -> Result<PrinterStatus> {
    unsafe {
        let printer_name = to_wide(printer);

        let mut handle = Default::default();

        let mut defaults = PRINTER_DEFAULTSW {
            pDatatype: PCWSTR::null(),
            pDevMode: std::ptr::null_mut(),
            DesiredAccess: PRINTER_ACCESS_USE.0,
        };

        OpenPrinterW(
            PCWSTR(printer_name.as_ptr()),
            &mut handle,
            Some(&mut defaults),
        )
        .map_err(|e| PrinterError::Message(e.to_string()))?;

        let mut needed = 0;

        GetPrinterW(handle, 2, None, 0, &mut needed);

        let mut buffer = vec![0u8; needed as usize];

        GetPrinterW(handle, 2, Some(buffer.as_mut_slice()), needed, &mut needed).map_err(|e| {
            ClosePrinter(handle);
            PrinterError::Message(e.to_string())
        })?;

        let info = &*(buffer.as_ptr() as *const PRINTER_INFO_2W);

        let status = info.Status;

        let mut reasons = Vec::<String>::new();
        let mut messages = Vec::<String>::new();

        macro_rules! add {
            ($flag:expr, $key:expr, $msg:expr) => {
                if status & $flag != 0 {
                    reasons.push($key.to_string());
                    messages.push($msg.to_string());
                }
            };
        }

        add!(PRINTER_STATUS_OFFLINE, "offline", "Máy in đang ngoại tuyến");

        add!(PRINTER_STATUS_PAPER_OUT, "media-empty", "Máy in hết giấy");

        add!(PRINTER_STATUS_PAPER_JAM, "media-jam", "Máy in bị kẹt giấy");

        add!(PRINTER_STATUS_DOOR_OPEN, "door-open", "Nắp máy in đang mở");

        add!(PRINTER_STATUS_PAUSED, "paused", "Máy in đang tạm dừng");

        add!(PRINTER_STATUS_TONER_LOW, "toner-low", "Mực in sắp hết");

        add!(PRINTER_STATUS_ERROR, "error", "Máy in đang gặp lỗi");

        add!(PRINTER_STATUS_BUSY, "busy", "Máy in đang bận");

        add!(PRINTER_STATUS_PRINTING, "printing", "Máy in đang in");

        add!(PRINTER_STATUS_PROCESSING, "processing", "Máy in đang xử lý");

        add!(
            PRINTER_STATUS_USER_INTERVENTION,
            "user-intervention",
            "Cần người dùng can thiệp"
        );

        add!(
            PRINTER_STATUS_OUTPUT_BIN_FULL,
            "output-full",
            "Khay giấy đầu ra đã đầy"
        );

        add!(
            PRINTER_STATUS_MANUAL_FEED,
            "manual-feed",
            "Máy in đang chờ nạp giấy thủ công"
        );

        add!(
            PRINTER_STATUS_IO_ACTIVE,
            "io-active",
            "Máy in đang truyền dữ liệu"
        );

        add!(PRINTER_STATUS_WAITING, "waiting", "Máy in đang chờ");

        add!(
            PRINTER_STATUS_WARMING_UP,
            "warming-up",
            "Máy in đang khởi động"
        );

        add!(
            PRINTER_STATUS_PENDING_DELETION,
            "pending-delete",
            "Máy in đang chờ xóa"
        );

        ClosePrinter(handle);

        Ok(PrinterStatus {
            state: status,
            reasons,
            messages,
            accepting: status & PRINTER_STATUS_PAUSED == 0,
        })
    }
}
